...
def commplex_exp_torch(phase, dtype=torch.complex64): # 将默认类型改为更常用的 complex64
    """
    ...
    # 使用 torch.complex 创建复数张量
    phase = torch.complex(real_part,imag_part)
    return phase
...
def ifftshift2d_tf(a_tensor):
    ...
    """
    # 获取张量的维度
    num_dims = len(a_tensor.shape)
    # 获取第二和第三维度的大小
    dim_size_2 = a_tensor.shape[-2]
    dim_size_3 = a_tensor.shape[-3]
    # 计算要移动的距离
    shift_2 = dim_size_2 // 2
    shift_3 = dim_size_3 // 2
    # 构建索引以进行移位
    indices = torch.arange(dim_size_2, dtype=torch.long, device=a_tensor.device)
    indices = (indices + shift_2) % dim_size_2
    # 在第二维度上进行移位
    a_tensor = torch.index_select(a_tensor, -2, indices)
    # 构建索引以进行移位
    indices = torch.arange(dim_size_3, dtype=torch.long, device=a_tensor.device)
    indices = (indices + shift_3) % dim_size_3
    # 在第三维度上进行移位
    a_tensor = torch.index_select(a_tensor, -3, indices)
    return a_tensor

def transp_ifft2d(a_tensor): # [FIX] 移除了未使用的 dtype 参数
    """
    通过transp_ifft2d将tensor的第二、第三维度进行逆傅里叶变换
    Useful functions:
    -   torch.fft.ifft2()
    """
    a_tensor = torch.fft.ifft2(a_tensor, dim=(-2, -3))
    return a_tensor
...
def transp_fft2d(a_tensor): 
    ...
    """
    # 对第二和第三维度进行傅里叶变换
    a_tensor = torch.fft.fft2(a_tensor, dim=(-2, -3))
    return a_tensor
